          ...
          <div className="p-6 flex-1">
            ...
            <nav className="space-y-2">
              <Link
                ...
                className={`block px-4 py-3 rounded-lg transition-colors ${isActive('/operational/dashboard')
                    ? 'bg-secondary-500/10 border border-secondary-500/20 text-secondary-300'
          ...
          <div className="p-6 border-t border-gray-200/10 dark:border-white/10">
            <div className="mb-3 p-3 rounded-lg bg-white/5 dark:bg-white/5">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-secondary-400 to-secondary-600 flex items-center justify-center text-white font-semibold text-sm">
                  {user?.full_name?.charAt(0).toUpperCase() || 'O'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-white truncate">
                    {user?.full_name || 'Operativo'}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
                    {user?.email || 'operativo@pms.com'}
                  </p>
                </div>
              </div>
            </div>
